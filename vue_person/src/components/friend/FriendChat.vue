...
export default {
  ...
  methods: {
    ...
    async handleMsgList (friendUserId) { // 获取聊天消息
      await this.$get('/api/friend/msg/friend', { friendUserId: friendUserId }).then((res) => {
    ...
    async getDataList () { // 获取好友列表
      ...
      const res = await this.$get('/api/friend/chat/friend')
      this.handleLoading(false)
      this.$store.commit('updateFriendList', res.data)
    },
    sendMsg () { // 发送消息
      this.$store.state.stomp.send('/ws/friend/chat', {}, JSON.stringify(this.chatFormat))
      this.chatFormat.msg = ''
      this.handleEditRead()
    },
    async handleEditRead () { // 修改信息阅读状态
      await this.$put('/api/friend/chat/read/friend', this.chatFormat)
      await this.$get('/api/friend/read/friend')
    }
  }
}
</script>
