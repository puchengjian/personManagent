...
export default {
  ...
  methods: {
    ...
    async getDataList () {
      ...
      const res = await this.$get('/api/friend/user/friend', this.queryListParam)
    ...
    async handleSearchFriend () {
      this.handleLoading(true)
      const res = await this.$get('/api/friend/user/friend/' + this.editFormData.phone)
      this.handleLoading(false)
    ...
    async handleSave () {
      this.handleLoading(true)
      const res = await this.$post('/api/friend/user/friend', this.editFormData)
      this.handleLoading(false)
      if (res.status !== 200) return
      this.getDataList()
      this.isTable = true
    ...
    async enterDel (id) {
      this.handleLoading(true)
      const res = await this.$delete('/api/friend/user/friend/' + id)
      this.handleLoading(false)
      if (res.status !== 200) return
      this.getDataList()
    }
